public class PatientList {
    ...
    public void removePatient(int id){
        Node current= head;
        Node prev= null;
        if(current!=null && current.getData().getId()==id){head=current.next;IO.println("Id number "+id+" deleted.");}
        while (current!=null&&current.getData().getId()!=id){prev=current;current=current.next;}
        if (current!=null && prev!=null){prev.next=current.next;IO.println("Id number "+id+" deleted.");}
